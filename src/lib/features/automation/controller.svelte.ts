...
import { defaultAction } from "./rule-defaults";
...
export function createAutomationRulesController() {
  ...
  async function testRule(): Promise<void> {
    if (testing || taskState.busy) return;
    testing = true;
    error = null;
    execution = null;
    try {
  ...
  async function runOnLibrary(): Promise<void> {
    if (selectedId === null || dirty || !preview || running || taskState.busy || preview.rowsNeedingChanges === 0) return;
    if (!window.confirm(
  ...
  return {
    ...
    get selectedRule() { return selectedRule; },
    get dirty() { return dirty; },
    get conditionCount() { return conditionCount; },
    get exportItems() { return exportItems; },
    refreshTags,
    copyAiRulePrompt,
    chooseRuleImport,
    openRuleTextImport,
