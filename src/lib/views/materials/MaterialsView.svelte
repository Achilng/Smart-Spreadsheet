<script lang="ts">
  ...
  $effect(() => { if (measuredWidth > 0) viewportWidth = measuredWidth; });
  const layout = $derived(galleryLayout(viewportWidth, app.galleryCardSize, items.length));
...
<section class="materials" inert={editorOpen}>
  ...
  <main>
    ...
    {:else}
      <GalleryViewport bind:measuredWidth spacerHeight={layout.spacerHeight} busy={loading}>
        {#each items as item, index (`${item.id}-${revision}`)}
          {@const position = galleryCellPosition(index, layout)}
          <GalleryTile x={position.x} y={position.y} width={layout.cardWidth} imageHeight={layout.imageHeight}
            title={item.title} tags={item.tags} isActive={selected?.id === item.id}
            onclick={() => selected = item} ondblclick={() => void copy(item)}>
            {#snippet image()}<Thumbnail rowId={item.id} loader={thumbnails} previewLoader={null} allowFileDrag={false} hasImage={true} alt={item.title} />{/snippet}
          </GalleryTile>
        {/each}
      </GalleryViewport>
    {/if}
