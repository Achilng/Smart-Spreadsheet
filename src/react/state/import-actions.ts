...
import { clearHistory, recordHistory } from "./history";

export async function updateAutoArtistPrefixOnImport(enabled: boolean): Promise<void> {
  if (useTasks.getState().busy || !useLibrary.getState().snapshot?.dataDirectory) return;
  useTasks.setState({ busy: true, label: "保存导入设置" });
  try {
    useLibrary.setState({ snapshot: await setAutoArtistPrefixOnImport(enabled) });
    setNotice({
      tone: "success",
      text: enabled
    ...
    useTasks.setState({ busy: false, label: "" });
  }
}
  ...
  await runAction(async () => {
    ...
    recordHistory({
      ...
      undo: async () => {
        ...
        await refreshLibrary({ snapshot: useLibrary.getState().snapshot! });
      },
      redo: async () => {
        ...
        if (result.added !== initial.added) {
          ...
          useLibrary.setState({ snapshot: cleanup.snapshot });
          await refreshLibrary({ snapshot: useLibrary.getState().snapshot! });
          throw new Error(
...
async function performImageImport(path: string, showResult: boolean): Promise<ImageImportResult> {
  const unlisten = await listen<ImageImportProgress>(
    ...
    event => {
      useTasks.setState({ progress: event.payload });
    },
  );
  ...
  try {
    ...
    useLibrary.setState({ snapshot: result.snapshot });
    if (result.added > 0) {
      await refreshLibrary({ snapshot: useLibrary.getState().snapshot!, resetScroll: true });
    }
    if (showResult) {
    ...
    unlisten();
    useTasks.setState({ progress: null });
