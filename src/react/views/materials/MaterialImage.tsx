...
export function MaterialImage({ id, loader, alt, expandable = false, active = true, passive = false }: { id: number; loader: ImageLoader; alt: string; expandable?: boolean; active?: boolean; passive?: boolean }) {
  ...
  return <div ref={container} className="rm-image"><LoadedImage key={`${id}-${attempt}`} id={active && (visible || lightbox) ? id : undefined} loader={loader} alt={alt} onOpen={expandable ? () => setLightbox(true) : undefined} onRetry={passive ? undefined : () => setAttempt(value => value + 1)} lightbox={active && lightbox} onClose={() => setLightbox(false)} /></div>;
}
