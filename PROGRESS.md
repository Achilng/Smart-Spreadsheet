...
第二轮迭代（v0.4）开发中。M15 导入内容去重、M16 通用删除与去重显示已完成并通过验收，当前推进 M17 Tag 交互方案 B。
...
- 自动化覆盖显式选择与筛选全选（含排除行）的覆盖计数、零覆盖 Tag 和 filtered 选择物化；当前 Rust 全量测试 113 项，Clippy 无警告，前端协议与生产构建通过。
