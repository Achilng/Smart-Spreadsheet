...
### 浅色前端重构视觉概念稿（2026-07-18）

- 新增独立静态概念稿 `design-prototypes/elegant-light-concept.html`，不接入现有
  Svelte 状态或 Tauri 命令，不改变当前应用运行界面。
- 视觉方向采用“暖白艺术档案馆 × 专业工作台”：以象牙白、暖灰、墨色和低饱和
  靛蓝建立层级，弱化通用蓝白后台感。
- 主窗口结构试验为窄导航、轻量筛选面板、三列图片画廊和紧凑检查器；图片卡片
  取消传统白色 footer，选择、VIBE 和序号使用轻量叠层或画册式标注。
- 右侧详情试验为“整理 / 提示词 / 文件”分区检查器；900px 最小宽度下自动隐藏
  检查器，把空间让给三列画廊。
- 概念稿完全离线，示意图片由内嵌 SVG 绘制；已使用 Playwright 在 1180×760
  默认窗口和 900×600 最小窗口下完成静态渲染检查。

### 以图搜图结果联动（2026-07-18）

- 相似图结果不再放置「查看详情」和「表格定位」按钮，整张结果卡片统一作为
  跳转入口。
- 工具箱通过定向窗口事件把目标行交给主窗口，并调用后端窗口命令恢复、显示和
  聚焦主窗口，不需要用户手动从任务栏来回查找。
- 点击卡片会清除 Tag、搜索、去重和分组等可能隐藏目标的条件，切到画廊后按
  当前窗口列数计算卡片坐标，将目标图片滚动到视区中央并设为当前详情记录。
- 新增按当前数据库行序计算位置的查询，不再用可能因删除而断号的行 ID 推算位置；
  已覆盖断号和不存在记录的 Rust 回归测试。
- 验证：Rust workspace 库测试 169 项通过、1 项性能基准忽略；Clippy
  workspace lib `-D warnings` 通过；`svelte-check` 0 错误（3 个既有图片拖拽
  可访问性警告），Vite 生产构建通过。

### 工具箱内部窗口修复（2026-07-18）

- 主窗口顶栏「工具箱」入口改用与「导入 / 导出」相同的通用按钮尺寸和字体，
  修复按钮高度及文字基线不对齐。
- 工具箱左上角仅保留纯文字「工具箱」，删除字母头像；系统窗口标题也同步简化，
  不再重复显示应用名称。
- 删除左侧栏「独立运行的小工具与资料库维护功能」说明，仅保留工具箱标题。
- 修复工具箱被系统浏览器打开并显示地址栏的问题：删除前端动态
  `new WebviewWindow` 和 `/?window=toolbox` URL 创建链路。
- 新增后端异步 `open_toolbox_window` 命令，使用
  `WebviewUrl::App("index.html?window=toolbox")` 明确加载应用内置前端资源；
  开发环境和正式构建均由 Tauri 应用窗口承载，不再交给默认浏览器。
- 单实例行为迁到后端：已有工具箱时恢复、显示并聚焦，否则创建与主窗口关联的
...
- 删除工具箱左下角的连接绿点、「已连接」状态和资料库图片数量，减少无操作价值的
  常驻信息。
...
### M53 — 工具箱窗口重构（2026-07-17）

- 已完整退役无实际价值的「建议分组」：删除顶栏入口、全屏视图、前端状态与
  API、Rust 命令、未分组键查询、相似度聚类管线及 `strsim` 依赖。
- 分组视图空状态改为提示用户选择图片后主动创建分组，不再引用已删除功能。
- 顶栏「工具」下拉菜单改为「工具箱」按钮；新增 960×680、最小 760×520 的
  无系统边框单实例 Tauri 子窗口，重复点击会恢复并聚焦现有窗口。
- 工具箱使用左侧分类导航和右侧工具面板，工具按需挂载且切换后保留临时状态；
  无资料库时只开放不依赖数据库的 JSON 文件工具。
- 随机画师串和智绘姬 JSON 去重从主窗口全屏浮层迁为工具箱内嵌面板；以图搜图
  改为在工具箱内完成选择、计算与结果展示，不再遮挡主资料库。
- 新增「资料库维护」面板，集中提供感知哈希刷新和失败图片目录；新增「数据管理」
  面板，隔离目录迁移与重置表格等高风险操作。
- 迁移或重置成功后通过 Tauri 定向事件通知主窗口重新读取快照；工具箱和主窗口
  的前端状态互相独立，不依赖共享 Svelte 单例。
- 「管理分组」退出通用工具列表，入口移到分组视图顶部工具栏。
- 权限拆为主窗口创建/聚焦子窗口能力和工具箱自身的窗口、事件、文件对话框能力。
- Playwright 以模拟 Tauri IPC 的真实浏览器页面检查 960×680 与 760×520 两种
  尺寸，导航、卡片、内容滚动和工具切换均无溢出。
- 验证：Rust 库测试 168 项通过、1 项性能基准忽略；`svelte-check` 0 错误
  （3 个既有图片拖拽可访问性警告）；Vite 生产构建通过；使用独立临时 target
  的 Tauri debug `--no-bundle` 构建通过；Clippy workspace lib
  `-D warnings` 通过。

### 画廊渐进式高清加载（2026-07-17）

- 图片显示链路拆为四级：256px 缩略图、1024px 画廊高清图、2048px
  详情预览和完整原图。
- 画廊继续立即显示缩略图；停止滚动 400ms 后，仅为真正处于视口内的卡片
  请求 1024px 高清图，以 2 个并发在后台生成；完整解码后直接替换，不添加
  淡入或额外加载动画。
- 重新滚动、切换视图或快速切换详情时会丢弃尚未开始的高清任务；已完成结果
  由分层 LRU 对象 URL 缓存复用，资料库内容变化时统一失效，运行中的旧请求
  不能再以复用的行 ID 写回新缓存。
- 虚拟画廊滚动导致卡片卸载、重建时会立即复用已经加载的 1024px 内存缓存；
  滚动只暂停新高清任务，不再把已变清晰的卡片主动降回缩略图。
- 高清层在 WebView 完成解码后再于下一绘制帧覆盖到底图上，底层缩略图始终保留，
  不再因两层同时切换产生空白帧；图片初始占位改为静态底色，不显示 shimmer 动画。
- 256/1024/2048 三档派生 PNG 均按原图路径、文件大小和修改时间生成独立磁盘
  缓存；缓存约超过 512MB 时分批清理最旧文件至约 448MB，避免无限增长。
- 详情面板会复用画廊已生成的 1024px 图片，并加载缓存化 2048px 版本；只有
  打开全屏查看时才直接读取完整 PNG 原始字节，不再由 Rust 解码和重新编码。
- 原生拖拽图标明确只选择 256px 缩略图缓存，避免误用新加入的高清缓存文件。
- 新增后端回归测试，验证三档派生尺寸分别缓存且完整原图字节不发生变化。
- 验证：Rust workspace 库测试 178 项通过、1 项性能基准忽略；Clippy
  workspace lib/bins `-D warnings` 通过；`svelte-check` 0 错误（4 个既有
  可访问性警告），Vite 生产构建通过。

### 画廊缩略图 VIBE 标识（2026-07-17）

- 画廊缩略图卡片右上角新增紫色 `VIBE ×N` 徽章，与详情面板一致显示原图中的 vibe 引用数量。
- 新增全应用共享的 VIBE 状态加载器：仅保留可见区排队任务、最多 4 个并发请求，并缓存最近 1000 行结果；打开详情时复用卡片已读取的状态，不重复解析。
- 资料库内容变化时同步清空 VIBE 状态缓存，避免行 ID 复用后显示旧徽章。
- 验证：`svelte-check` 0 错误（4 个既有可访问性警告），Vite 生产构建通过。

### 画廊右侧详情图片适配修复（2026-07-17）

- 详情面板预览区改为明确的响应式画布，并同时约束图片最大宽度与最大高度。
- 横图、竖图和非方形原图均按原始宽高比完整缩放居中，不再因预览元素尺寸与原图比例不一致而只显示局部。
- 预览画布会随窗口高度在 180–320px 之间调整，避免挤压下方详情字段。
- 验证：`svelte-check` 0 错误（4 个既有可访问性警告），Vite 生产构建通过。

### Tag 输入建议列表滚动修复（2026-07-17）

- 详情面板的 Tag 输入建议不再只截取前 8 项，所有符合搜索条件且尚未贴到当前图片的 Tag 均可选择。
- 建议下拉列表限制为 240px 高并支持独立纵向滚动；滚轮到达列表边界时不会继续带动详情面板滚动。
- 验证：`svelte-check` 0 错误（4 个既有可访问性警告），Vite 生产构建通过。

### M52 — 仅保留 XLSX 导出，清除电子表格遗留链（2026-07-17）

- 数据库 schema 升至 v10：新库直接创建当前结构，不再依次执行 v1–v9 的工作簿迁移。
- 现有 v8/v9 库可原地升级；历史电子表格批次统一转为只读 `legacy` 来源，
  行、Tag、分组和受管图片路径保持不变。
- v1–v7 库不再由当前版本继续背负升级实现，打开时返回明确的旧版升级提示。
- 删除电子表格读取器、内嵌图提取器、待提取队列表、`workbook/` 运行时依赖、
  示例工作簿、遗留集成测试，以及直接依赖 `calamine` / `quick-xml`。
- XLSX 导出完整保留；测试改为直接验证 OOXML 包结构、文字字段、绘图锚点、
  缺图降级、拒绝覆盖和空选择错误。
- 导入页文案只保留 PNG、文件夹和压缩包入口。
- 验证：Rust workspace 全目标测试 177 项通过、1 项性能基准忽略；
  Clippy workspace 全目标 `-D warnings` 通过；`svelte-check` 0 错误（4 个既有
  a11y 警告）；Vite 生产构建通过。

### 原图搬家重关联与受管原图识别（2026-07-17）

- 修复文件夹来源完整受管副本被误判为“无元数据 XLSX 缩略图”的问题：数据库 schema 升至 v9，新增 `stored_image_is_original` 行级可信标记；旧 xlsx 嵌入图保持不可信，文件夹和压缩包的完整副本可在外部原路径失效后继续用于拖出。
- 新增稳定的 NovelAI 完整元数据指纹：对 `Description` 原文与完整 `Comment` JSON 计算 SHA-256；JSON 递归排序对象键并忽略无意义空白，seed、采样参数、V4 角色信息和 vibe/reference 等字段均参与匹配。
- 新图片入库时同步保存 `metadata_fingerprint`；资料库打开时只从外部完整原图或可信受管原图回填历史行，旧 xlsx 无元数据缩略图不会参与。
- “导入 → 更新现有图片”匹配链升级为“原身份路径 → 完整文件 SHA-256 → 完整元数据指纹”：SHA 唯一命中可覆盖原图改名/搬家，文件重新保存但完整生成元数据一致时仍可重新关联。
- 重关联会同步更新身份键、当前原图路径、提示词、文件与感知哈希、元数据指纹，并把候选刷新/升级为完整受管原图；没有旧副本的行会创建 `files/relinked/row-<id>.png`。
- 同一文件内容或元数据指纹命中多条旧行时不自动选择，结果提示单独报告“匹配到多条记录”；未匹配来源仍不会新增进资料库。
- 前端更新说明弹窗和完成提示已展示内容重关联、元数据重关联与歧义数量。
- 新增回归覆盖：文件夹完整副本在外部路径失效后仍可解析为原图、SHA 搬家重关联、重新保存后的完整元数据重关联、多候选拒绝自动覆盖、schema v9 字段与历史指纹回填。
- 验证：Rust lib 181 项通过、1 项性能基准忽略；Clippy `--lib --bins -D warnings` 通过；全目标 Clippy 仍被既有 `tests/map_embedded_images.rs` 引用已退役 API 的编译错误阻断；`svelte-check` 0 错误（4 个既有 a11y 警告），Vite 生产构建通过。

### 撤销/重做与删除体验（2026-07-15）

- 删除成功后改为 stale-while-revalidate 原位刷新，不再清空滚动位置或主动回顶；导入、换库等整体数据变化仍按原规则回顶。
- 删除弹窗的「同时将原始图片文件移入回收站」改为每次默认勾选，并醒目说明删除不进入撤销历史、应用内无法恢复。
- `Delete` 键保持现有安全语义：优先作用于选中行，无选择时作用于详情行，输入控件内不触发。
- 新增会话级通用操作历史：最近 50 步，撤销失败会回填撤销栈，重做失败会回填重做栈，新操作会清空重做分支。
- 工作区全局接入 `Ctrl+Z` / `Ctrl+Shift+Z`，并兼容 `Ctrl+Y`；输入框、文本域和富文本仍使用浏览器原生撤销，删除确认框开启时不触发后台历史。
- 新增行可变状态快照 API：提示词/备注/画师串/Tag/分组关系在单个 SQLite 事务中批量恢复，任一行不存在时整批回滚；新增回归测试验证完整恢复和原子回滚。
- 图片导入结果新增批次 ID：撤销精确删除该批次行/受管副本/空批次记录，来源原图不动；重做重新导入同一路径并更新批次 ID。
- 单行正向/角色/负向提示词、备注、详情 Tag，批量查找替换/画师前缀，侧边栏 Tag 打标/删除，分组成员分配及分组新建/重命名/删除/空分组清理均已接入操作历史。
- 图片删除和「更新现有图片」不可撤销，成功后清空会话历史；换库、迁移、重置数据同样清空历史。
- 「新建并分配分组」和「应用建议分组」将多个建组/成员分配子操作合并为单步历史，一次 `Ctrl+Z` 即撤销整个用户操作，不受 50 步历史上限影响。
- 验证：Rust lib 172 项通过、1 项性能基准忽略；Clippy `-D warnings` 通过；`svelte-check` 0 错误（4 个既有 a11y 警告）；Vite 生产构建通过。

v_21 已发布。画册功能移除与 WebView 原生右键菜单屏蔽已随 0.9.5 交付。

### v0.9.5 — 移除画册与精简右键菜单（2026-07-11）

- 应用版本提升至 0.9.5；前端检查 0 错误（4 个既有 a11y 警告）、Vite 生产构建、Cargo Check、Clippy 严格检查和 Windows x64 NSIS 打包通过；Rust 库内测试 164 项通过（1 项性能基准忽略）。
- 安装器 `Smart-Spreadsheet_0.9.5_x64-setup.exe` 大小 4,420,369 字节，SHA-256 `D94D1FF237F4E4F066934CEC04D3A0B7483A9149A503A836A25835AB5B7890C3`。
- GitHub Release `v_21` 已发布，详细说明包含画册功能退役范围、自定义右键菜单使用位置、覆盖安装方式和校验信息。

### 移除画册 + 屏蔽 WebView 原生右键菜单（2026-07-11）

- 完整移除画册导航、保活视图、浏览/阅读组件、缓存 store、前端 API、Tauri 命令和 Runtime 入口。
- 将原 `albums.ts` 中仍在使用的画师名单、随机画师串和“选中相同画师串”接口迁移到 `artists.ts`，这些非画册能力不受影响。
- 重复聚合恢复为固定至少 2 个成员，删除仅为画册单张成册服务的 `min_members` 参数与测试分支。
- 在应用根级阻止 `contextmenu` 默认行为，初始化页与工作区都不再弹出 WebView 原生菜单；现有自定义右键菜单仍由目标组件正常打开。
- 验证：前端检查 0 错误（4 个既有 a11y 警告），Vite 生产构建通过，`cargo check --workspace` 通过，Rust 库内测试 164 项通过（1 项性能基准忽略）。全量 workspace 测试仍被既有 `tests/map_embedded_images.rs` 引用已退役 xlsx 嵌图 API 的编译错误阻断，与本次改动无关。

### v0.9.4 — 角色提示词独立展示（2026-07-11）

- 应用版本提升至 0.9.4；Rust 164 项测试通过（1 项性能基准忽略）、Clippy 严格检查通过、前端检查 0 错误（保留既有 5 个 a11y 警告）、Vite 生产构建和 Windows x64 NSIS 打包通过，启动冒烟测试通过。
- 安装器 `智能表格_0.9.4_x64-setup.exe` 大小 4,419,532 字节，SHA-256 `1F76D0E87BF1CA63E2BCC5FBC1991E8FEE80C9B22B05E7BFCDF6D48F8163FC40`。
- GitHub Release `v_20` 已发布，详细说明包含功能入口、旧资料库迁移方式和“更新现有图片”操作步骤。

### 角色提示词独立字段（2026-07-11）

- 撤销把 `v4_prompt.caption.char_captions[*].char_caption` 拼入正向提示词的行为：基础正向提示词、角色提示词、负向提示词现在分别解析和存储，多角色描述仍按原顺序以换行连接。
- 数据库升级至 schema v7，新增 `rows.character_prompt`；历史行不做启发式拆分，避免误判用户原有换行，使用“导入 → 更新现有图片”即可从原 PNG 准确回填。
- 查询 API、全文搜索、xlsx 导出、详情面板和表格视图同步支持角色提示词；详情中可独立复制、编辑角色提示词。
- 画师串仍综合基础正向提示词与角色提示词提取，编辑任一字段都会基于两者重新计算，避免角色描述中的画师标签丢失。
- 智绘姬 JSON 的 `fixedPrompt` 继续只导出基础正向提示词，不混入角色提示词。
- 验证：Rust 164 项测试通过（1 项性能基准忽略）、Clippy 严格检查通过、前端检查 0 错误（保留既有 5 个 a11y 警告）、Vite 生产构建通过。

### v0.9.3 — V4 角色描述与更新现有图片（2026-07-11）

- 应用版本提升至 0.9.3；Rust 162 项测试通过（1 项性能基准忽略）、Clippy 严格检查通过、前端检查 0 错误（保留既有 5 个 a11y 警告）、Vite 生产构建和 Windows x64 NSIS 打包通过，启动冒烟测试通过。
- 安装器 `智能表格_0.9.3_x64-setup.exe` 大小 4,415,631 字节，SHA-256 `52C2471563EEEA17E0C614F60772CE32C883B85AE77E94D6D188C0D336EA4587`。
- GitHub Release `v_19` 已发布，更新说明为一句话。

### 更新现有图片（2026-07-11）

- “导入”菜单新增“更新现有图片”，先显示规则说明弹窗，再选择原图片文件夹或原压缩包；弹窗明确说明只更新、不新增，并列出 Tag/分组保留、缺失图片不删除、失败保留旧数据等规则。
- 后端沿用现有身份键精确匹配：文件夹图片按规范化绝对路径，压缩包图片按压缩包路径＋包内路径；仅原位更新已匹配行的正负提示词（含 V4 角色描述）、画师串、内容哈希、感知哈希和来源变化字段，不创建新行或导入批次。
- 同步刷新已匹配行的受管原图副本与缩略图缓存；来源中新图片直接计入“未匹配并忽略”，元数据读取或副本刷新失败时不覆盖原行。
- 完成提示分别报告发现、匹配、更新、未匹配忽略、元数据失败和副本失败数量。
- 新增数据库与文件夹/压缩包端到端存储回归测试，验证提示词更新、新图片不追加、失败保留旧提示词，以及行 ID、Tag、分组和受管副本保持正确。

### NovelAI V4 角色描述解析修复（2026-07-11）

- PNG 元数据解析现在会把 `v4_prompt.caption.char_captions[*].char_caption` 中所有非空角色描述，按原顺序追加到基础正向提示词中，多段之间使用换行分隔。
- 没有基础提示词、仅有角色描述的 V4 图片也能正常入库；画师串会基于合并后的完整正向提示词重新提取。
- 新增回归测试覆盖多角色、空角色描述、角色描述内画师标签，以及仅角色描述四种情况。
...
### 空备注导出确认与序号回退 - 2026-07-14

- 智绘姬 JSON 导出新增备注预检 command；前端在打开保存窗口前检查当前导出范围，并统计空备注数量。
- 存在空备注时显示 native 警告弹窗：“继续导出”会让空项使用其导出位置 `1 / 2 / 3...` 命名，“取消”不会打开保存窗口或写出文件。
- 非空备注继续作为 preset 键；最终名称仍统一检查重复，包括“非空备注恰好等于另一空项的数字回退名称”的冲突，避免覆盖预设。
- 自动化测试覆盖未确认时拒绝、确认后数字回退、普通重名和数字回退冲突；`npm run check` 0 错误，生产构建通过，Rust 库内 168 项测试通过、1 项忽略。

### Release v_23 - 2026-07-14

- 使用仓库内 Tauri CLI 完成 Windows x64 release 优化构建，生成包含空备注导出确认修复的 NSIS 安装包。
- 导出智绘姬 JSON 时会在保存前检查空备注：用户可取消，或继续并让空项按导出位置使用数字名称；最终名称冲突仍会阻止导出。
- 发布前验证：`svelte-check` 0 错误、4 个既有可访问性警告；Vite 生产构建通过；Rust 库内 168 项测试通过、忽略 1 项。
- 安装包大小为 4,415,452 字节，SHA-256 为 `1B47C33CF0E34E89EA255316273BCBFDC872F39E89C3ADA2CB089643BA271478`。
- 以 ASCII 文件名 `Smart-Spreadsheet_0.9.5_x64-setup.exe` 上传至 GitHub Release `v_23`：`https://github.com/Achilng/Smart-Spreadsheet/releases/tag/v_23`。
...
- XLSX 只允许生成式导出，不读取、修改或依赖任何工作簿副本。
- 不安装全局 `cargo-tauri`；使用仓库内 `@tauri-apps/cli`，Node 版本保持不变。

### 前端概念 C「柔光画室」- 2026-07-19

- 新增独立可交互设计稿 `design-prototypes/concept-c-soft-atelier.html`，未修改现有三份概念稿。
- 视觉语言改为暖燕麦纸、鼠尾草绿、杏桃色与低饱和灰紫；使用圆润异形圆角、半透明纸张层次、编辑式错落画廊和植物拼贴装饰，降低传统后台 / 开发工具感。
- 信息架构大胆调整为“收藏室”语义：左侧品牌与轻导航、中间午后画册、右侧作品一览；提示词与参数降级为折叠内容，作品预览和标签优先。
- 新增原创植物纸张背景与作品示例图两项本地资产；设计稿不依赖在线图片服务。
- 补充卡片聚焦、批量选择计数、筛选切换、视图切换、收藏、复制、搜索回车与收录按钮等演示交互。
- 使用 Chromium 在 1440×900 视口完成视觉检查；修复网格子项最小高度导致的侧栏溢出，并让批量操作条稳定悬浮在画廊底部。
- 浏览器快照验证作品切换会同步右侧标题与尺寸，勾选后批量计数由 2 更新为 3，筛选与复制提示正常；页面无脚本错误。

### 前端概念 D「琉璃画廊」- 2026-07-20

- 按 `D:\Agent\playground\weather-card.html` 的视觉语言新增独立玻璃拟态设计稿 `design-prototypes/concept-d-prism-glass.html`；概念 C 保留，不覆盖历史方案。
- 采用蓝青到灰紫的动态环境光、半透明厚玻璃、细白描边、内高光、漂浮光团、扫光与轻微指针视差，并提供晨雾 / 晚霞 / 夜航三套可切换主题。
- 信息架构重组为窄浮动导航、收藏集筛选条、错落 Bento 作品墙、独立作品详情玻璃舱与底部悬浮批量操作坞；主作品图片成为首屏视觉中心。
- 使用一张本地原创作品图作为主作品和详情预览，其余卡片以 CSS 绘制抽象色彩练习；页面无外部图片服务依赖。
- 实现作品聚焦与详情同步、选择计数、收藏、筛选、视图切换、环境光切换、提示词复制、搜索和收录提示等演示交互。
- 使用 Chromium 在 1440×900 视口对照参考页完成视觉检查；浏览器快照确认主题切换、作品详情同步与选择计数由 2 更新为 3，控制台 0 错误、0 警告。
