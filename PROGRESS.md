...
- 每行新增自定义右键菜单，可输入一个大小写敏感 Tag 并通过 Enter 或按钮添加；操作始终构造只包含被右键行 ID 的显式选择，不读取或修改当前多选。
- 浏览器验收确认预先选中两行后，右键第三行只为第三行新增 `DirectTag`，全局计数为 1 且多选仍为 2 行。
- 右键菜单支持空输入校验，并在 Escape、点击外部、表格滚动、筛选重载或组件销毁时关闭。
- 开发服务器端口由 `1420` 调整为当前未占用的 `127.0.0.1:1422`，Vite 启动参数与 Tauri `devUrl` 保持一致；正式构建不依赖该端口。

## 进行中

- 无；首轮反馈中的启动、窗口、表格和右键 Tag 优化均已完成。
