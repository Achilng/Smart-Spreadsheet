...
第二轮迭代（v0.4）开发中，当前推进 M15 导入内容去重。schema v3 迁移已完成，下一步实现历史图片哈希补算与导入管线内容去重。

## 已完成

### M15 - 导入内容去重（进行中，2026-06-12）
...
- Rust 全量测试 108 项通过；Clippy 无警告；`svelte-check` 0 错误 0 警告，前端生产构建通过。
